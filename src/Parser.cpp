...
bool parseInstruction(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseType(content, pos, type)) {
        if (type == "sphere") {
            ...
            return parseMaterial(content, pos, object);
        }
    }
...
bool parseSphere(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseVector(content, pos, position) &&
        parseVector(content, pos, rotation) &&
        parseVector(content, pos, scale) &&
        parseColor(content, pos, color) &&
        parseScalar(content, pos, angle) &&
        parseType(content, pos, material_name)) {
        object.reset(new Sphere(position, rotation, scale, color, angle * 0.0175, material_name));
        return true;
    }
...
bool parseTriangle(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    Vector color;
    std::string material_name;
    if (parsePoint(content, pos, p1) &&
        parsePoint(content, pos, p2) &&
        parsePoint(content, pos, p3) &&
        parseColor(content, pos, color) &&
        parseType(content, pos, material_name)) {
        object.reset(new Triangle(p1, p2, p3, color, material_name));
        return true;
    }
...
bool parseRectangle(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseVector(content, pos, position) &&
        ...
        parseScalar(content, pos, angle) &&
        parseType(content, pos, material_name)) {
        object.reset(new Rectangle(position, rotation, scale, color, angle * 0.0175, material_name));
        return true;
    }
...
bool parseCylinder(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseVector(content, pos, position) &&
        ...
        parseScalar(content, pos, angle) &&
        parseType(content, pos, material_name)) {
        object.reset(new Cylinder(position, rotation, scale, color, angle * 0.0175, material_name));
        return true;
    }
    return false;
}
...
bool parseCone(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseVector(content, pos, position) &&
        ...
        parseScalar(content, pos, angle) &&
        parseType(content, pos, material_name)) {
        object.reset(new Cone(position, rotation, scale, color, angle, material_name));
        return true;
    }
    return false;
}
...
static bool parseCamera(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseScalar(content, pos, depth) && parseVector(content, pos, position) &&
        parseVector(content, pos, orientation)) {
...
static bool parseMaterial(std::string const& content, size_t& pos, std::unique_ptr<Object>& object) {
    ...
    if (parseType(content, pos, name) &&
        parseScalar(content, pos, coefReflection) &&
        parseScalar(content, pos, transparence) &&
        parseScalar(content, pos, indiceRefraction) &&
        parseScalar(content, pos, brillance)) {
        object.reset(new Materiaux(name, coefReflection, indiceRefraction, brillance, transparence));
        return true;
    }
    return false;
}
...
bool parseColor(std::string const& content, size_t& pos, Vector& color) {
    ...
    if (content[pos] == '(') {
        ++pos;
        if ((parseInt(content, pos, r) && r <= 255 && r >= 0) && (parseInt(content, pos, g) && g <= 255 && g >= 0) &&
            (parseInt(content, pos, b) && b <= 255 && b >= 0)) {
            consumeBlank(content, pos);
            if (content[pos] == ')') {
                ++pos;
                color = Vector(r, g, b);
